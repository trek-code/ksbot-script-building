...
$sourceDir = Join-Path $workspace "scripts\rs-perk-farmer-v103\src"
$classDir = Join-Path $workspace "build\rs-perk-farmer-v103\classes"
$packageDir = Join-Path $workspace "handoff\packages"
$serverSlug = $Server.ToLower().Replace("-", "")
$jarName = "rs-perk-farmer-v103-" + $serverSlug + "-" + $Version + ".jar"
$jarPath = Join-Path $packageDir $jarName
$latestName = "rs-perk-farmer-v103-" + $serverSlug + "-latest.jar"
...
Write-Host "Built rs-perk-farmer v103 package:"
Write-Host $jarPath
if ($deployDir) {
