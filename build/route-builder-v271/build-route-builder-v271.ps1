...
$workspace = "D:\Code Agents\Codex GPT\RSPS\KSBOT Script building"
$sourceDir = Join-Path $workspace "scripts\route-builder-v271\src"
$classDir = Join-Path $workspace "build\route-builder-v271\classes"
$packageDir = Join-Path $workspace "handoff\packages"
$serverSlug = $Server.ToLower().Replace("-", "")
$jarName = "route-builder-cl-" + $serverSlug + "-" + $Version + ".jar"
$jarPath = Join-Path $packageDir $jarName
$latestName = "route-builder-cl-" + $serverSlug + "-latest.jar"
...
switch ($Server) {
    "Reason" {
        ...
        $deployDir = "C:\Users\jonez\.kreme\servers\Reason\scripts\route_builder"
    }
    "Near-Reality" {
        ...
        $deployDir = "C:\Users\jonez\.kreme\servers\Near-Reality\scripts\route_builder"
    }
}
...
Write-Host "Built route-builder CL package:"
Write-Host $jarPath
if ($deployDir) {
