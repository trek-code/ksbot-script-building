...
$sourceDir = Join-Path $workspace "scripts\rs-perk-farmer-cl-refined-v2\src"
$classDir = Join-Path $workspace "build\rs-perk-farmer-cl-refined-v2\classes"
$packageDir = Join-Path $workspace "handoff\packages"
$serverSlug = $Server.ToLower().Replace("-", "")
$jarName = "rs-perk-farmer-cl-refined-v2-" + $serverSlug + "-" + $Version + ".jar"
$jarPath = Join-Path $packageDir $jarName
$latestName = "rs-perk-farmer-cl-refined-v2-" + $serverSlug + "-latest.jar"
...
Write-Host "Built perk-farmer CL refined v2 package:"
Write-Host $jarPath
if ($deployDir) {
