...
$sourceDir = Join-Path $workspace "scripts\rs-perk-farmer\src"
$classDir = Join-Path $workspace "build\rs-perk-farmer\classes"
$packageDir = Join-Path $workspace "handoff\packages"
$serverSlug = $Server.ToLower().Replace("-", "")
$jarName = "rs-perk-farmer-cl-" + $serverSlug + "-" + $Version + ".jar"
$jarPath = Join-Path $packageDir $jarName
$latestName = "rs-perk-farmer-cl-" + $serverSlug + "-latest.jar"
...
Write-Host "Built rs-perk-farmer CL package:"
Write-Host $jarPath
if ($deployDir) {
