...
$sourceDir = Join-Path $workspace "scripts\\pf-wbh-v125\\src"
$classDir = Join-Path $workspace "build\\pf-wbh-v125\\classes"
$packageDir = Join-Path $workspace "handoff\\packages"
$serverSlug = $Server.ToLower().Replace("-", "")
$jarName = "pf-wbh-runite-ore-" + $serverSlug + "-" + $Version + ".jar"
$jarPath = Join-Path $packageDir $jarName
$latestName = "pf-wbh-runite-ore-" + $serverSlug + "-latest.jar"
...
Write-Host "Built PF (WBH) Runite ore package:"
Write-Host $jarPath
if ($deployDir) {
