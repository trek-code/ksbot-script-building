...
$sourceDir = Join-Path $workspace "scripts\rs-perk-farmer-cl-world-boss-handler-test\src"
$classDir = Join-Path $workspace "build\rs-perk-farmer-cl-world-boss-handler-test\classes"
$packageDir = Join-Path $workspace "handoff\packages"
$serverSlug = $Server.ToLower().Replace("-", "")
$jarName = "rs-perk-farmer-cl-world-boss-handler-test-" + $serverSlug + "-" + $Version + ".jar"
$jarPath = Join-Path $packageDir $jarName
$latestName = "rs-perk-farmer-cl-world-boss-handler-test-" + $serverSlug + "-latest.jar"
...
Write-Host "Built perk-farmer CL world-boss-handler test package:"
Write-Host $jarPath
if ($deployDir) {
