...
public final class ReasonWorldBossHandler {
    ...
    /** True after our first successful "Attack" click this fight; lets us
     *  distinguish FIGHT_ENGAGE (initial click) from ATTACK_BOSS (re-clicks). */
    private volatile boolean firstAttackFired = false;
    ...
    private int tickBankPreset() {
        ...
        switch (presetStep) {
            case 0: // Open preset UI
                closeInterfaces();
                sleep(DelayProfile.Category.BANKING);
    ...
    private int tickTeleport() {
        ...
        sleep(DelayProfile.Category.TELEPORT);
    ...
    private int tickWaitSpawn() {
        ...
        // If we died while standing at the spawn tile (boss one-shot on spawn,
        // nearby mob, PvP, etc.) bounce straight back through WALK_TO_BOSS
        // instead of sitting here until spawnTimeout fires.
        if (detectAndHandleDeath(playerTile())) return 600;
        ...
        if (!preSpawnPrepped && now >= preSpawnPrepAtMs) {
            ...
            doActivatePrayers();
            sleep(DelayProfile.Category.PRAYER);
            doDrinkPotion();
        ...
        if (boss != null && boss.isAlive()) {
            ...
            try {
                bot.webhook.send(DiscordWebhook.Event.BOSS_DETECTED,
                        activeBoss.displayName + " has spawned — engaging.");
            } catch (Throwable ignored) {}
            if (!preSpawnPrepped) {
    ...
    private void doDrinkPotion() {
        try {
            ...
            if (potion != null) {
                ...
                sleep(DelayProfile.Category.POTION);
            } else {
    ...
    private int tickFight() {
        ...
        // Death check FIRST — before timeout and before null-boss==killed.
        // If we died, the server teleports us far from the arena so the boss
        // NPC is no longer in our scene (findBossNpc would return null). Also,
        // if we died late in a long fight, the fightTimeoutMs may have expired
        // in the meantime — without this guard we'd either (a) wrongly credit
        // a kill and walk to loot, or (b) "time out" to loot — in both cases
        // we end up at the loot tile instead of running back to re-engage.
        WorldPoint meNow = playerTile();
        if (detectAndHandleDeath(meNow)) return 600;
        ...
        if (boss == null || boss.isDead()) {
            ...
            try {
                BossStats st = statsFor(activeBoss);
                bot.webhook.send(DiscordWebhook.Event.BOSS_KILLED,
                        activeBoss.displayName + " defeated. Session: "
                                + st.killed + " kills / " + st.attempted + " attempts.");
            } catch (Throwable ignored) {}
            disableBossPrayers();
            snapshotInventory();
            fightStartedAt = 0L;
        ...
        // (Death detection moved above the null-boss check — see earlier block.)
        WorldPoint me = meNow;
        ...
        if (!ok) {
            fail("Failed to attack " + activeBoss.displayName);
            return bot.delays.roll(DelayProfile.Category.ATTACK_BOSS);
        }
        // First successful click of this fight gets the FIGHT_ENGAGE window
        // (typically short — simulates the quick commit on spawn); subsequent
        // re-clicks get ATTACK_BOSS.
        if (!firstAttackFired) {
            firstAttackFired = true;
            return bot.delays.roll(DelayProfile.Category.FIGHT_ENGAGE);
        }
        return bot.delays.roll(DelayProfile.Category.ATTACK_BOSS);
    }
    ...
    private int tickLoot() {
        ...
        if (lootSweeps == 1) {
            int settleMs = bot.delays.roll(DelayProfile.Category.POST_KILL_SETTLE);
            bot.log("[world-boss] Waiting " + settleMs + "ms for ground loot to appear...");
            sleep(settleMs);
            recordAutoLoot();
        }
        ...
        try {
            ...
            for (KSGroundItem item : nearby) {
                ...
                trackLoot(name, qty);
                found = true;
                sleep(DelayProfile.Category.LOOT_PICKUP);
            }
    ...
    private void trackLoot(String name, int qty) {
        ...
        try {
            if (bot.webhook.isRareDrop(name)) {
                bot.webhook.send(DiscordWebhook.Event.RARE_DROP,
                        "Rare drop: " + name + " x" + qty
                                + (activeBoss != null ? " (from " + activeBoss.displayName + ")" : ""));
            }
        } catch (Throwable ignored) {}
    }
    ...
    /**
     * If the player is clearly far from the boss arena (death teleported us
     * back to the respawn tile), credit a death, reset prep + waypoint state,
     * disable prayers, and push the phase to WALK_TO_BOSS so we run back and
     * re-engage. Returns true if a death was detected + handled, false
     * otherwise. Callers should early-return after a true to let the new
     * phase take over on the next tick.
     */
    private boolean detectAndHandleDeath(WorldPoint me) {
        if (activeBoss == null) return false;
        if (me == null || activeBoss.arrivalTile == null) return false;
        ...
        int d = me.distanceTo(bossTile);
        if (d <= 20) return false;
        bot.log("[world-boss] Appears we died — respawned far from boss (dist="
                + d + "). Re-walking + re-potting.");
    ...
    /** Sleep a category-randomized amount from the bot's DelayProfile. */
    private void sleep(DelayProfile.Category cat) {
        sleep(bot.delays.roll(cat));
    }
}
