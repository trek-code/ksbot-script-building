...
public final class RouteBuilderPanel extends JFrame {
    ...
    private static final Color ORANGE = new Color(185, 95, 40);
    ...
    private final JLabel resourceNameLabel = liveVal("---");
    private final JLabel resourceIdLabel   = liveVal("---");
    private final JLabel resourceXLabel    = liveVal("---");
    private final JLabel resourceYLabel    = liveVal("---");
    private final JLabel resourcePLabel    = liveVal("-");
    ...
    private JPanel buildLiveCaptureBar() {
        ...
        JPanel resourceRow = new JPanel(new FlowLayout(FlowLayout.LEFT, 6, 0));
        resourceRow.setOpaque(false);
        resourceRow.add(sectionLabel("Nearest Resource:"));
        JComboBox<String> resourceFilterCombo = new JComboBox<>(RouteBuilderBot.RESOURCE_FILTER_OPTIONS);
        resourceFilterCombo.setSelectedItem(bot.getResourceFilter());
        resourceFilterCombo.setToolTipText("Filter which kind of resource the 'Nearest Resource' row tracks.");
        resourceFilterCombo.addActionListener(e -> {
            Object sel = resourceFilterCombo.getSelectedItem();
            if (sel != null) bot.setResourceFilter(sel.toString());
        });
        resourceRow.add(labeled("Filter:", resourceFilterCombo));
        resourceRow.add(labeled("Name:", resourceNameLabel));
        resourceRow.add(labeled("ID:",   resourceIdLabel));
        resourceRow.add(labeled("X:",    resourceXLabel));
        resourceRow.add(labeled("Y:",    resourceYLabel));
        resourceRow.add(labeled("P:",    resourcePLabel));
        JButton refreshResource = btn("Refresh Resource", BLUE);
        refreshResource.addActionListener(e -> bot.onRefreshNearestResource());
        resourceRow.add(refreshResource);
        resourceRow.add(copyBtn("Copy ID",       () -> copyToClipboard(resourceIdLabel.getText())));
        resourceRow.add(copyBtn("Copy X",        () -> copyToClipboard(resourceXLabel.getText())));
        resourceRow.add(copyBtn("Copy Y",        () -> copyToClipboard(resourceYLabel.getText())));
        resourceRow.add(copyBtn("Copy XYP",      () -> copyToClipboard(resourceXLabel.getText() + ", " + resourceYLabel.getText() + ", " + resourcePLabel.getText())));
        resourceRow.add(copyBtn("Copy ID,X,Y,P", () -> copyToClipboard(resourceIdLabel.getText() + " @ " + resourceXLabel.getText() + ", " + resourceYLabel.getText() + ", " + resourcePLabel.getText())));
        JButton addResourceRow = btn("+ Row: Resource", ORANGE);
        addResourceRow.addActionListener(e -> bot.onAddNearestResourceRow());
        resourceRow.add(addResourceRow);
    ...
    public void onLiveUpdate(RouteBuilderBot.LiveSnapshot snap) {
        ...
        SwingUtilities.invokeLater(() -> {
            try {
                ...
                if (latest.nearestResourceValid) {
                    resourceNameLabel.setText(truncate(latest.nearestResourceName, 20));
                    resourceIdLabel.setText(String.valueOf(latest.nearestResourceId));
                    resourceXLabel.setText(String.valueOf(latest.nearestResourceX));
                    resourceYLabel.setText(String.valueOf(latest.nearestResourceY));
                    resourcePLabel.setText(String.valueOf(latest.nearestResourcePlane));
                } else {
                    resourceNameLabel.setText("---"); resourceIdLabel.setText("---");
                    resourceXLabel.setText("---");    resourceYLabel.setText("---"); resourcePLabel.setText("-");
                }
    ...
    public void addRowFromNearestResource(String name, int id, int x, int y, int plane, String actions) {
        SwingUtilities.invokeLater(() -> {
            tileModel.addRow(new Object[]{name == null ? "resource" : name, id, x, y, plane, actions == null ? "" : actions, "resource"});
            absorbCellValueIntoDropdown(namesModel, name);
            if (id > 0) absorbCellValueIntoDropdown(idsModel, String.valueOf(id));
            absorbCellValueIntoDropdown(actionsModel, actions);
            absorbCellValueIntoDropdown(notesModel, "resource");
            scrollToLastRow();
        });
    }
