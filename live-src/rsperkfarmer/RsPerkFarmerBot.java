 ...
 */
@ScriptManifest(
        name        = "RS Perk Farmer CL v1.00",
        author      = "you",
        version     = 1.00,
        uid         = "kreme.rs.perkfarmer.cl",
...
public class RsPerkFarmerBot extends Script {
    ...
    public int onProcess() {
        ...
        // and race back to the Perk Master for the next task.
        checkTaskCompletionChat();
    ...
    private void tickChooseResource() {
        ...
        // We ALWAYS want the runite one, whichever slot it is — and we parse
        // the number so we can stop mining the instant the task completes.
        ...
        try {
            ...
            for (int i = 0; i < opts.size(); i++) {
                ...
                String txt = w != null ? w.getText() : null;
                if (txt != null && txt.toLowerCase().contains("runite")) {
                    runiteText = txt;
                    runiteIndex = i;
                    break;
                }
        ...
        boolean picked;
        if (runiteIndex >= 0) {
            picked = ctx.dialog.chooseOption(runiteIndex);
        } else {
            // Last-ditch: predicate form (may or may not be wired on every server).
            Predicate<String> isRunite = text -> text != null && text.toLowerCase().contains("runite");
            picked = ctx.dialog.chooseOption(isRunite);
        }
        if (!picked) { failStep("No 'runite' option in task menu"); return; }
    ...
    private void tickMining() {
        ...
        if (taskTargetCount > 0 && taskMinedThisTask >= taskTargetCount) {
            log("Quota reached (" + taskMinedThisTask + "/" + taskTargetCount + ") — banking.");
            setState(BotState.WALK_TO_BANK);
            return;
        }
    ...
    private KSNPC findSpringCreature() {
        try {
            return ctx.npcs.query()
                    .withName("Spring Creature", "Spring creature", "Spring")
                    .withinDistance(10)
                    .closest();
        } catch (Throwable ignored) {
            return null;
    ...
    /**
     * Looks for "You have completed your perk task and received N perk
     * experience." in the last few seconds of chat. When found, records the
     * XP, bumps the completion counter, resets task state, and jumps the
     * state machine straight toward banking (or the Perk Master if the
     * inventory is already clean).
     */
    private void checkTaskCompletionChat() {
        // Ignore in states where we haven't accepted a task yet or we are
        // mid-dialog — dialog states end before we start mining.
        BotState s = state;
        ...
        String line = latestCompletionLine();
        if (line == null) return;

        long xp = 0;
        Matcher m = TASK_COMPLETE_PATTERN.matcher(line);
        if (m.find()) {
        ...
        tasksCompleted++;
        log("Perk task COMPLETE — +" + xp + " perk xp (session total: " + totalPerkXpEarned + ").");
        ...
        // If we're mid-mining with ore in the bag, bank on the way back —
        // otherwise cut straight back to the Perk Master.
        if (hasAnyRuneOre()) {
            setState(BotState.WALK_TO_BANK);
        } else {
            setState(BotState.WALK_TO_NPC);
    ...
    /**
     * Returns the most recent chat line containing the completion signature,
     * or null if none within the last ~6 seconds. We keep the window small so
     * we don't re-trigger on a stale line while processing a later state.
     */
    private String latestCompletionLine() {
        try {
            Object msg = ctx.chat.query()
                    .contains("completed your perk task")
                    .inLastSeconds(6)
                    .first();
            if (msg == null) return null;
            try {
                // KSChatMessage.getMessage() — confirmed via API jar.
                return (String) msg.getClass().getMethod("getMessage").invoke(msg);
            } catch (Throwable t) {
                return msg.toString();
            }
        } catch (Throwable ignored) {
    ...
    private void ensureWindow() {
        ...
        Runnable build = () -> {
            JFrame f = new JFrame("RS Perk Farmer CL v1.00");
    ...
    public void log(String line) {
        System.out.println("[RS-PerkFarmer-CL] " + line);
