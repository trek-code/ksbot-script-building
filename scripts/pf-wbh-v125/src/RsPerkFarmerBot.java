 ...
 */
@ScriptManifest(
        name        = "PF (WBH) Runite ore v1.51",
        author      = "you",
        version     = 1.51,
        uid         = "kreme.rs.perkfarmer.pfwbh.runiteore.v151",
        description = "Perk Farmer CL — Elite Skilling perk tasks + world-boss handler test bed.",
...
public class RsPerkFarmerBot extends Script implements BotContextGates {
    ...
    private volatile long lastStateChangeMs = 0L;
    private volatile long lastMineAtMs      = 0L;
    ...
    // ── Staff tracking data classes ────────────────────────────────────────
    public static final class StaffEntry {
        public final String name;
        public final int    distance;
        StaffEntry(String n, int d) { name = n; distance = d; }
    }
    public static final class StaffChatEntry {
        public final String  sender;
        public final String  message;
        public final long    timestampMs;
        public final boolean nearbyAtSend;
        public final boolean isStaff;
        /** "Help", "Yell", "Public", or "Staff" */
        public final String  channel;
        StaffChatEntry(String s, String m, long t, boolean n, boolean staff, String ch) {
            sender = s; message = m; timestampMs = t; nearbyAtSend = n;
            isStaff = staff; channel = ch;
        }
    }

    // ── Staff tracking fields ──────────────────────────────────────────────
    private volatile int                    staffOnlineCount = 0;
    private volatile int                    staffNearbyCount = 0;
    private volatile int                    lastStaffNearbyCount = 0;   // for edge-trigger webhook
    private volatile List<StaffEntry>       staffNearbyList  = new java.util.ArrayList<>();
    private volatile String[]               staffAllList     = new String[0];
    private final    List<StaffChatEntry>   staffChatLog     =
            java.util.Collections.synchronizedList(new java.util.ArrayList<>());
    private static final int STAFF_CHAT_MAX = 200;
    ...
    /** Which bank preset slot (1-5) holds the user's mining gear (with pickaxe).
     *  Used by the world-boss handler when re-gearing after a kill. */
    private volatile int     miningPresetSlot = 2;
    ...
    /** Shared world-boss handler — opt-in via the UI panel checkbox. */
    private final ReasonWorldBossHandler worldBossHandler = new ReasonWorldBossHandler(this);

    /** Per-category anti-ban delay profile (min/max ms per action type).
     *  Tuned via the "Delays" tab in the panel. See {@link DelayProfile}. */
    public final DelayProfile delays = new DelayProfile();

    /** Smart break scheduler. See {@link BreakManager}. */
    public final BreakManager breaks = new BreakManager();

    /** Discord webhook client for event notifications. */
    public final DiscordWebhook webhook = new DiscordWebhook();

    /** Last delay category that fired (for the paint strip). */
    private volatile DelayProfile.Category lastDelayCategory = null;
    /** Actual ms value rolled for {@link #lastDelayCategory}. */
    private volatile int lastDelayMs = 0;
    /** Rolling action counter — used for actions/hour paint stat. */
    private volatile int actionCounter = 0;
    private volatile long actionCounterStartedAtMs = System.currentTimeMillis();

    public DelayProfile.Category getLastDelayCategory() { return lastDelayCategory; }
    public int getLastDelayMs()                         { return lastDelayMs; }
    public int getActionsPerHour() {
        long elapsed = System.currentTimeMillis() - actionCounterStartedAtMs;
        if (elapsed <= 0) return 0;
        return (int) (actionCounter * 3_600_000L / elapsed);
    }
    ...
    public boolean onStart() {
        ...
        // Auto-load persisted webhook config (URL + enabled events + attach set
        // + screenshot settings) so it survives script reloads.
        if (webhook.tryLoadDefault()) {
            log("[webhook] Loaded config from " + DiscordWebhook.DEFAULT_CONFIG_PATH);
        }
        ensureWindow();
        try {
        ...
        // Register known world bosses — off by default, UI checkbox enables.
        worldBossHandler.register(BossConfig.VOTE_BOSS);
        worldBossHandler.register(BossConfig.BLOOD_REAPER);
        worldBossHandler.register(BossConfig.STONE_POLTEGEIST);
        worldBossHandler.register(BossConfig.DONATION_BOSS);
        worldBossHandler.register(BossConfig.SPRING_BOSS);
        worldBossHandler.setGlobalEnabled(false);

        log("Script loaded. Press Start in the RS Perk Farmer panel to begin.");
        return true;
    }
    ...
    public int onProcess() {
        ...
        // ── Break manager tick — short-circuit if we're on break. ────────
        // The manager is wired in lazily on first onProcess so the gates +
        // logout/resume actions can reference `this` post-construction.
        if (breaks.getState() == BreakManager.State.IDLE
                && breaks.quick == null /* never true — keep compiler happy */) {}
        ensureBreakManagerWired();
        boolean suspend = breaks.tick(System.currentTimeMillis());
        // Keep DelayProfile.rampActive in sync with BreakManager state.
        delays.setRampActive(breaks.isRamp());
        if (suspend) {
            setStatus("On break: " + (breaks.getActiveConfig() == null
                    ? "?" : breaks.getActiveConfig().kind));
        ...
        // Always refresh nearby player count and staff data so paint/UI stay live.
        worldBossHandler.refreshNearbyPlayerCount();
        refreshStaffData();
        ...
        // World-boss interrupt — higher priority than spring creature. If the
        // handler says "interrupt", save state and enter the WORLD_BOSS phase.
        // We don't interrupt during dialogs or if already handling the boss.
        if (worldBossHandler.shouldInterrupt()
                && state != BotState.WORLD_BOSS && state != BotState.FAULT) {
            log("[world-boss] Interrupting state " + state + " for boss event.");
            stateBeforeSpring = state;   // reuse the "restore" field
            setState(BotState.WORLD_BOSS);
        }
        // If the handler finished, resume the saved state.
        if (state == BotState.WORLD_BOSS && worldBossHandler.isDone()) {
            worldBossHandler.acknowledge();
            log("[world-boss] Boss routine complete. Resuming " + stateBeforeSpring);
            setState(stateBeforeSpring == BotState.WORLD_BOSS
                    ? BotState.CHECK_EQUIPMENT : stateBeforeSpring);
        }

        // Spring creature check runs on every tick except when we're already in
        // the middle of handling one, or when we're faulted out or boss-ing.
        if (state != BotState.SPRING_EVENT && state != BotState.FAULT
                && state != BotState.WORLD_BOSS) {
        ...
        try {
            switch (state) {
                ...
                case LOAD_MINING_PRESET:  tickLoadMiningPreset(); break;
                ...
                case WORLD_BOSS:          return worldBossHandler.tick();
    ...
    private void onChatMessageEvent(ChatMessage evt) {
        try {
            ...
            // 0. Forward to world-boss handler so it can detect boss notifications.
            worldBossHandler.onChatMessage(lower);
            ...
            if (lower.contains("reached perk tree level")) {
                ...
                if (m.find()) {
                    try {
                        ...
                        // Only webhook if it's a real bump (not baseline).
                        if (prev > 0 && lvl > prev) {
                            try {
                                webhook.send(DiscordWebhook.Event.LEVEL_UP,
                                        "Perk tree level " + lvl
                                                + (awarded > 0 ? " (+" + awarded + " perk points)" : ""));
                            } catch (Throwable ignored) {}
                        }
            ...
            // 4. Channel + staff chat capture.
            //    Always record [Help] and [Yell] messages (server-wide channels visible
            //    to everyone). Also always record any message from a known staff member
            //    in any channel. Staff messages are flagged with isStaff=true.
            try {
                String sender = evt.getName();
                String channel = null;

                // Detect [Help] channel — message text carries the "[Help]" prefix.
                if (lower.startsWith("[help]") || lower.contains("[help]")) channel = "Help";
                // Detect yell — various prefixes used by RSPS servers.
                else if (lower.startsWith("[yell]") || lower.startsWith("[::yell]")
                        || lower.contains("[yell]")) channel = "Yell";

                boolean isStaff = false;
                if (sender != null && !sender.isBlank()) {
                    try { isStaff = ctx.antiBan.isStaffMember(sender); } catch (Throwable ignored) {}
                    if (isStaff && channel == null) channel = "Staff";
                }

                // Record if it's a monitored channel OR a staff member speaking anywhere.
                if (channel != null && sender != null && !sender.isBlank()) {
                    boolean nearbyNow = false;
                    List<StaffEntry> snap = staffNearbyList;
                    for (StaffEntry e : snap) {
                        if (sender.equalsIgnoreCase(e.name)) { nearbyNow = true; break; }
                    }
                    StaffChatEntry entry = new StaffChatEntry(
                            sender, clean, System.currentTimeMillis(),
                            nearbyNow, isStaff, channel);
                    synchronized (staffChatLog) {
                        staffChatLog.add(entry);
                        while (staffChatLog.size() > STAFF_CHAT_MAX) staffChatLog.remove(0);
                    }
                    log("[chat-" + channel + "]" + (isStaff ? "[STAFF]" : "")
                            + " " + sender + ": " + clean);
                    if (isStaff) {
                        try {
                            webhook.send(DiscordWebhook.Event.STAFF_CHAT,
                                    "[" + channel + "] " + sender + ": " + clean);
                        } catch (Throwable ignored) {}
                    }
                }
            } catch (Throwable ignored) {}
    ...
    /** Unused — force-load preset removed. Falls through to CHECK_EQUIPMENT. */
    private void tickLoadMiningPreset() {
        setState(BotState.CHECK_EQUIPMENT);
    }
    ...
    private void tickChooseTaskType() {
        ...
        if (!ctx.dialog.isViewingOptions()) {
            ...
            scheduleIn(DelayProfile.Category.NPC_DIALOGUE);
            return;
        }
    ...
    private void tickMining() {
        ...
        if (curInv > lastSeenRuneOreInv) {
            ...
            int before = oresMined;
            taskMinedThisTask += gained;
            oresMined         += gained;
            lastMineAtMs = System.currentTimeMillis();
            // MILESTONE — fire when this gain pushes us across an N-bucket boundary.
            try {
                int n = webhook.getMilestoneEveryN();
                if (n > 0 && (oresMined / n) > (before / n)) {
                    int crossed = (oresMined / n) * n;
                    webhook.send(DiscordWebhook.Event.MILESTONE,
                            "Mined " + crossed + " runite ore this session.");
                }
            } catch (Throwable ignored) {}
        }
        ...
        consecutiveFails = 0;
        scheduleIn(DelayProfile.Category.MINE_ROCK);
    }
    ...
    private void tickWalkToBank() {
        ...
        if (me != null && me.distanceTo(RouteProfile.BANK_CHEST_TILE) > 15) {
            ...
            scheduleIn(DelayProfile.Category.WALK_MOVEMENT);
            return;
        }
    ...
    private void tickBanking() {
        if (!ctx.bank.isOpen()) {
            ...
            scheduleIn(DelayProfile.Category.BANKING);
            return;
        }
    ...
    private void checkTaskCompletion() {
        ...
        tasksCompleted++;
        try {
            webhook.send(DiscordWebhook.Event.TASK_COMPLETED,
                    "Perk task completed (+" + xp + " perk xp). Total tasks: " + tasksCompleted);
        } catch (Throwable ignored) {}
    ...
    private volatile boolean breakManagerWired = false;

    private void ensureBreakManagerWired() {
        if (breakManagerWired) return;
        breakManagerWired = true;
        breaks.setGates(this);
        breaks.setLogoutAction(this::attemptLogout);
        breaks.setResumeAction(() -> log("[break] Resuming after break."));
        breaks.setEventLog(s -> {
            log("[break] " + s);
            if (s != null && s.startsWith("Break START"))
                webhook.send(DiscordWebhook.Event.BREAK_STARTED, s);
            else if (s != null && s.startsWith("Break END"))
                webhook.send(DiscordWebhook.Event.BREAK_ENDED, s);
        });
    }

    /** Best-effort logout: try the logout tab widget. Falls back to stop()
     *  idle if the widget isn't available (still safe — we just stay logged
     *  in for the break duration). */
    private void attemptLogout() {
        try {
            // Open the logout tab — widget group 182, child 9 is the logout
            // button in standard OSRS interface. On Reason/Near Reality the
            // same IDs typically apply because both derive from RuneLite.
            // If it fails, we degrade to AFK (no action taken).
            KSWidget logoutBtn = null;
            try { logoutBtn = ctx.widgets.getWidget(182, 9); } catch (Throwable ignored) {}
            if (logoutBtn != null) {
                logoutBtn.interact("Logout");
                log("[break] Logout widget clicked.");
            } else {
                log("[break] Logout widget not found — staying AFK.");
            }
        } catch (Throwable t) {
            log("[break] Logout attempt threw: " + t.getMessage());
        }
    }

    // ── BotContextGates implementation (server-agnostic break gates) ──────
    @Override public boolean isInCombat() {
        try {
            return ctx.players.getLocal().getInteracting() != null;
        } catch (Throwable t) { return false; }
    }

    @Override public boolean isInventoryFullOfLoot() {
        try { return ctx.inventory.isFull(); } catch (Throwable t) { return false; }
    }

    @Override public boolean isMidTask() {
        return taskTargetCount > 0 && taskMinedThisTask < taskTargetCount;
    }

    @Override public boolean isInSafeState() {
        switch (state) {
            case IDLE:
            case WALK_TO_NPC:
    ...
    @Override public boolean isStaffNearby() {
        try { return ctx.antiBan.staffNearby(8); } catch (Throwable t) { return false; }
    }

    @Override public boolean isLowHp() {
        try { return ctx.skills.getBoostedLevel(net.runelite.api.Skill.HITPOINTS) <= 20; }
        catch (Throwable t) { return false; }
    }
    ...
    private void setState(BotState next) {
        if (this.state != next) {
            ...
            this.lastStateChangeMs = this.stateEnteredAt;
        }
    }

    /** Category-aware scheduleIn: picks a random value in the category's
     *  configured [min, max] window, then applies normal scheduleIn semantics
     *  (including any global antiban jitter on top). Records last-delay info
     *  for the paint strip and bumps the actions/hour counter. */
    void scheduleIn(DelayProfile.Category cat) {
        int ms = delays.roll(cat);
        this.lastDelayCategory = cat;
        this.lastDelayMs       = ms;
        this.actionCounter++;
        scheduleIn(ms);
    }
    ...
    private void scheduleIn(long ms) {
        ...
        // Idle / AFK jitter: probabilistic long pause on top of everything else.
        int idleExtra = delays.rollIdleJitterMs();
        if (idleExtra > 0) {
            log("[antiban] Idle-jitter pause: " + idleExtra + "ms");
        }
        this.lastAntibanExtraMs = extra;
        this.nextActionAtMs = System.currentTimeMillis() + ms + extra + idleExtra;
    }
    ...
    private void fault(String reason) {
        ...
        try { webhook.send(DiscordWebhook.Event.FAULT, reason); }
        catch (Throwable ignored) {}
    }
    ...
    // ── Staff data refresh ────────────────────────────────────────────────
    /** Throttle so we don't hit ctx.antiBan.getStaff() / nearbyStaff() every
     *  game tick — those calls do scene scans and (on some servers) network
     *  lookups, and refreshing twice a second is plenty for the UI. */
    private static final long STAFF_REFRESH_INTERVAL_MS = 2_000L;
    private volatile long lastStaffRefreshAtMs = 0L;
    private void refreshStaffData() {
        long now = System.currentTimeMillis();
        if (now - lastStaffRefreshAtMs < STAFF_REFRESH_INTERVAL_MS) return;
        lastStaffRefreshAtMs = now;
        try {
            String[] all = ctx.antiBan.getStaff();
            staffAllList     = all != null ? all : new String[0];
            staffOnlineCount = staffAllList.length;
        } catch (Throwable ignored) {}
        try {
            List<KSPlayer> nearby = ctx.antiBan.nearbyStaff(16).list();
            staffNearbyCount = nearby != null ? nearby.size() : 0;
            List<StaffEntry> entries = new java.util.ArrayList<>();
            if (nearby != null) {
                WorldPoint me = playerTile();
                for (KSPlayer p : nearby) {
                    if (p == null) continue;
                    String  name = p.getName();
                    WorldPoint loc = p.getWorldLocation();
                    int dist = (me != null && loc != null) ? me.distanceTo(loc) : -1;
                    entries.add(new StaffEntry(name != null ? name : "?", dist));
                }
            }
            staffNearbyList = entries;

            // Edge trigger: fire only when nearby count rises (0→1, 1→2, etc.)
            // — avoids spamming every tick once a staffer is in range.
            if (staffNearbyCount > lastStaffNearbyCount) {
                StringBuilder names = new StringBuilder();
                for (int i = 0; i < entries.size() && i < 5; i++) {
                    if (i > 0) names.append(", ");
                    names.append(entries.get(i).name);
                    if (entries.get(i).distance >= 0) {
                        names.append(" (").append(entries.get(i).distance).append("t)");
                    }
                }
                try {
                    webhook.send(DiscordWebhook.Event.STAFF_NEARBY,
                            staffNearbyCount + " staff nearby: " + names);
                } catch (Throwable ignored) {}
            }
            lastStaffNearbyCount = staffNearbyCount;
        } catch (Throwable ignored) {}
    }
    ...
    public void paint(Graphics g) {
        try {
            ...
            try {
                ...
                Font stripFont = new Font(Font.SANS_SERIF, Font.BOLD,  11);

                // ── DEDICATED TOP STRIP: delays + break + idle info ──────────
                // Rendered ABOVE the normal row block, in its own box, so the
                // user can see the anti-ban machinery working at a glance.
                long now = System.currentTimeMillis();

                DelayProfile.Category lastCat = lastDelayCategory;
                String delayStr = lastCat == null
                        ? "—"
                        : lastCat.label + " " + lastDelayMs + "ms";

                long waitMs0 = nextActionAtMs - now;
                String nextInStr = waitMs0 <= 0 ? "now" : formatMs(waitMs0);

                BreakManager.State bst = breaks.getState();
                String breakStr;
                switch (bst) {
                    case ACTIVE: {
                        long rem = breaks.getBreakEndAtMs() - now;
                        breakStr = "ON BREAK " + formatMs(Math.max(0, rem));
                        break;
                    }
                    case RAMP: {
                        long rem = breaks.getRampEndAtMs() - now;
                        breakStr = "RAMP " + formatMs(Math.max(0, rem));
                        break;
                    }
                    case PENDING: breakStr = "PENDING"; break;
                    default: {
                        long nq = breaks.getNextQuickAtMs();
                        long nl = breaks.getNextLongAtMs();
                        long next = 0;
                        if (nq > 0 && (next == 0 || nq < next)) next = nq;
                        if (nl > 0 && (next == 0 || nl < next)) next = nl;
                        breakStr = next == 0 ? "idle" : "in " + formatMs(next - now);
                    }
                }

                int aph = getActionsPerHour();
                String idleStr = delays.isIdleEnabled() ? "ON" : "off";

                // Compose two strip lines: [delay | next] and [break | a/hr | idle]
                String stripLine1 = "Delay: " + delayStr + "   Next: " + nextInStr;
                String stripLine2 = "Break: " + breakStr + "   A/hr: " + aph + "   Idle: " + idleStr;

                // Rows: label + value. Build first, then size the box.
                long waitMs = nextActionAtMs - now;
                String nowLine;
                if (state == BotState.WORLD_BOSS) {
                    nowLine = worldBossHandler.statusText();
                } else if (antibanEnabled && waitMs > 0 && lastAntibanExtraMs > 0
                        && !isPaused() && state != BotState.FAULT) {
                    nowLine = "Delayed " + lastAntibanExtraMs + "ms (antiban)";
                } else {
                    nowLine = state.currentAction();
                }
                String[][] rows = new String[][] {
                        ...
                        {"Players near:", String.valueOf(worldBossHandler.getNearbyPlayerCount())},
                        {"Staff nearby:", String.valueOf(staffNearbyCount)},
                        {"Staff online:", String.valueOf(staffOnlineCount)},
                        {"Last mine:",   lastMineAtMs == 0 ? "—"
                                : formatMs(System.currentTimeMillis() - lastMineAtMs) + " ago"},
                        {"Last action:", lastStateChangeMs == 0 ? "—"
                                : formatMs(System.currentTimeMillis() - lastStateChangeMs) + " ago"},
                ...
                // ── Top strip (2 lines) ─────────────────────────────────────
                int stripLineH = 14;
                int stripH = padY + 2 * stripLineH + padY;
                int stripY = PAINT_Y;

                g2.setColor(PAINT_BG);
                g2.fillRoundRect(PAINT_X, stripY, PAINT_W, stripH, 8, 8);
                g2.setColor(new Color(120, 180, 220));  // cyan-ish border to distinguish
                g2.drawRoundRect(PAINT_X, stripY, PAINT_W, stripH, 8, 8);
                g2.setFont(stripFont);
                g2.setColor(new Color(180, 220, 255));
                int stripTextY = stripY + padY + stripLineH - 3;
                g2.drawString(stripLine1, PAINT_X + 8, stripTextY);
                g2.drawString(stripLine2, PAINT_X + 8, stripTextY + stripLineH);

                // ── Main box (offset below the strip) ────────────────────────
                int boxY = stripY + stripH + 4;
                int height  = padY + titleH + rows.length * rowH + padY;

                // Box
                g2.setColor(PAINT_BG);
                g2.fillRoundRect(PAINT_X, boxY, PAINT_W, height, 8, 8);
                g2.setColor(PAINT_BORDER);
                g2.drawRoundRect(PAINT_X, boxY, PAINT_W, height, 8, 8);

                // Title
                g2.setFont(titleFont);
                g2.setColor(PAINT_TITLE);
                g2.drawString("PF (WBH) Runite ore", PAINT_X + 8, boxY + 16);

                // Rows
                int y = boxY + titleH + padY;
    ...
    private void ensureWindow() {
        ...
        Runnable build = () -> {
            JFrame f = new JFrame("PF (WBH) Runite ore v1.51");
    ...
    public void log(String line) {
        System.out.println("[PFWBH151] " + line);
    ...
    public long getLastStateChangeMs()       { return lastStateChangeMs; }
    public long getLastMineAtMs()            { return lastMineAtMs; }
    ...
    public int  getMiningPresetSlot() { return miningPresetSlot; }
    public void setMiningPresetSlot(int slot) {
        if (slot < 1) slot = 1;
        if (slot > 5) slot = 5;
        this.miningPresetSlot = slot;
    }
    public int     getLastAntibanExtraMs() { return lastAntibanExtraMs; }
    public ReasonWorldBossHandler getWorldBossHandler() { return worldBossHandler; }
    public int             getStaffOnlineCount()  { return staffOnlineCount; }
    public int             getStaffNearbyCount()  { return staffNearbyCount; }
    public String[]        getStaffAllList()       { return staffAllList; }
    public List<StaffEntry> getStaffNearbyList()  { return staffNearbyList; }
    public List<StaffChatEntry> getStaffChatLog() {
        synchronized (staffChatLog) { return new java.util.ArrayList<>(staffChatLog); }
    }
    ...
    public void userStart() {
        ...
        try { webhook.send(DiscordWebhook.Event.BOT_STARTED, "Script is live."); }
        catch (Throwable ignored) {}
    }
    ...
    public void userStop() {
        ...
        try {
            String summary = "Tasks: " + tasksCompleted + ", ores: " + oresMined
                    + ", uptime: " + (sessionStartMs == 0 ? "0"
                            : ((System.currentTimeMillis() - sessionStartMs) / 1000) + "s");
            webhook.send(DiscordWebhook.Event.BOT_STOPPED, summary);
        } catch (Throwable ignored) {}
    }
